...
export interface Food {
  ...
  serve: ServeMap;      // подача по возрасту (кусочки/BLW — базовая)
  servePuree?: ServeMap;
  /** Отдельно вводимые виды внутри карточки (например, орехи): каждый со своим статусом в дневнике. */
  variants?: { key: string; label: string }[]; // подача для кормления пюре (показывается, если мама выбрала «пюре»)
}
